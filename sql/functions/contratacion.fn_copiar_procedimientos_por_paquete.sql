CREATE OR REPLACE FUNCTION contratacion.fn_copiar_procedimientos_por_paquete(_negociacion_actual_id bigint, _negociacion_anterior_id bigint, _sede_prestador_id integer, _codigo_portafolio character varying)
...
DECLARE
    _total_count bigint;
BEGIN
    INSERT INTO contratacion.sede_negociacion_paquete_procedimiento(procedimiento_id, sede_negociacion_paquete_id,
                                                                    cantidad, principal, cantidad_minima,
                                                                    cantidad_maxima, costo_unitario, costo_total,
                                                                    observacion, ingreso_aplica, ingreso_cantidad,
                                                                    frecuencia_unidad, frecuencia_cantidad)
    SELECT snpp_anterior.procedimiento_id,
           snp_actual.id AS sede_negociacion_paquete_id,
           snpp_anterior.cantidad,
           snpp_anterior.principal,
    ...
    FROM contratacion.sedes_negociacion sn_anterior
             ...
             INNER JOIN contratacion.sede_negociacion_paquete_procedimiento snpp_anterior
    ...
    RAISE NOTICE 'Se insertaron % procedimientos en la negociacion', _total_count;
