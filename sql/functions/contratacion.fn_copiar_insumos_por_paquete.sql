CREATE OR REPLACE FUNCTION contratacion.fn_copiar_insumos_por_paquete(_negociacion_actual_id bigint, _negociacion_anterior_id bigint, _sede_prestador_id integer, _codigo_portafolio character varying)
...
DECLARE
    _total_count bigint;
BEGIN
    INSERT INTO contratacion.sede_negociacion_paquete_insumo (insumo_id, sede_negociacion_paquete_id, cantidad,
                                                              observacion, cantidad_minima, cantidad_maxima,
                                                              costo_unitario, costo_total, ingreso_aplica,
                                                              ingreso_cantidad, frecuencia_unidad, frecuencia_cantidad)
    SELECT snpp_anterior.insumo_id,
    ...
    FROM contratacion.sedes_negociacion sn_anterior
             ...
             INNER JOIN contratacion.sede_negociacion_paquete_insumo snpp_anterior
    ...
    RAISE NOTICE 'Se insertaron % insumos en la negociacion', _total_count;
    RETURN concat('Insumos clonados de la negociacion', _negociacion_actual_id, _negociacion_anterior_id);
END;
$function$
;
